...
class AlquilerController extends Controller
{
     ...
     * Lists all Test entities.
     *
     * @Route("/banner/{id}/{codigo}/{banner}", name="wizard_start_banner")
     * @Method({"GET"})
     */
    public function startAlquilerBannerAction(Request $request, $id, $codigo,$banner)
    {
        $em = $this->getDoctrine()->getManager();
        $oferta = $em->getRepository('AppBundle:Oferta')->getOfertaActivaByCodeAndAuto($codigo,$id);

        if(!$oferta){
            $ofertas = $em->getRepository('AppBundle:Oferta')->getOfertaAutos(1);
            $oferta = $ofertas[0];
            $ofertaAuto = $oferta->getOfertaAutos()[0];

            $b = $em->getRepository('AppBundle:Banner')->find($banner);
            $b->setOferta($oferta);
            $b->setOfertaAuto($ofertaAuto);
            $em->flush();

            $auto = $oferta->getOfertaAutos()[0]->getAuto();

            $request->getSession()->set('car_selected', $auto->getId());
            $request->getSession()->set('offert_code', $oferta->getCodigo());
        }else{
            $request->getSession()->set('car_selected', $id);
            $request->getSession()->set('offert_code', $codigo);
        }
